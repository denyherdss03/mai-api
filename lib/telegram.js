  ...
  return {
    inline_keyboard: [
      [
        ...
        { text: '🔴 PAGO NO VÁLIDO', callback_data: `invalido:${orderId}` },
      ],
    ],
  };
}
...
export async function sendOrderToTelegram(orderData) {
  ...
  if (!botToken || !chatId) throw new Error('Variables de entorno de Telegram no configuradas.');
...
async function sendPhotoToTelegram({ botToken, chatId, caption, imageBuffer, imageName, replyMarkup }) {
  ...
  form.append('photo', imageBuffer, { filename: imageName || 'comprobante.jpg', contentType: 'image/jpeg' });
  try {
    await axios.post(url, form, { headers: form.getHeaders(), maxContentLength: Infinity, maxBodyLength: Infinity, timeout: 15000 });
  } catch (error) {
...
async function sendMessageToTelegram({ botToken, chatId, caption, replyMarkup }) {
  ...
  try {
    await axios.post(url, { chat_id: chatId, text: caption, parse_mode: 'Markdown', reply_markup: replyMarkup }, { timeout: 15000 });
  } catch (error) {
    const details = error.response?.data ? JSON.stringify(error.response.data) : error.message;
